...
def percent():
    ...
    while True:
        try:
            punc_file_to_open =Path(input("\nYOU SELECTED OPTION 9: CALCULATE ERROR PERCENTAGE. Please, insert your file path and wait for instructions: "))
            punc_dic_to_open=Path(input('\nPlease, insert your dictionary path: '))
            with open(punc_file_to_open,'r', encoding="utf-8") as f:
                freq = wordpunct_tokenize(f.read())
            with open (punc_dic_to_open,'r', encoding="utf-8") as fr:
...
def lex_info():
    ...
    def lex_dens():
        ...
        print('This text contains',len(set(words)), 'types')
        print('The lexical richness (type/token ratio) of this text is: ',len(set(words))/len(words))
    ...
    def percentage():
        ...
        print(100 * words.count(us) / len(words))
